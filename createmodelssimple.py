...
numfiles = len(files)
i = 0
R2results = pd.DataFrame(np.zeros((numfiles, 4)), columns=["SVM", "NN", "Ada", "GPR"])
models = pd.DataFrame(
    np.full((numfiles, 4), np.nan), columns=["SVM", "NN", "Ada", "GPR"]
)
for file in files:
    ...
    i = i + 1
pickle.dump(R2results, open("R2results.p", "wb"))
